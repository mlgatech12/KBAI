...
def solve():
    ...
    with open("AgentAnswers.csv","w") as results:     # Results will be written to ProblemResults.csv.
        ...
        i=1                                               # Do not write anything else to ProblemResults.txt during execution of the program.
        results.write("ProblemSet,RavensProblem,Agent's Answer,Answer\n")
        for set in sets:
            for problem in set.problems:   # Your agent will solve one problem at a time.
                #try:
                #answer = -1
                #if(problem.name == "Challenge Problem E-03"):
                ...
                i+=1
                #print("answer = ", i , "=" , str(answerArr))
                #results.write("%s,%s,%d,%s\n" % (set.name, problem.name, answer, str(answerArr)))
